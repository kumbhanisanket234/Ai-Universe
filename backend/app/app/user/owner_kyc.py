...
from .forms.owner_kyc import user_kyc_model
...
async def owner_kyc(token:str = Depends(oauth2_scheme) , user_kyc : user_kyc_model = Form()):
    ...
    if getuser:
        ...
        cur.execute("select * from user_kyc where email = %s", email)
        user = cur.fetchone()
        ...
        if user:
            ...
            image_path = UPLOAD_DIR / user_kyc.panImage.filename
            with open(image_path, "wb") as f:
                f.write(await user_kyc.panImage.read())

            image_path2 = UPLOAD_DIR / user_kyc.aadharImage.filename
            with open(image_path, "wb") as f:
                f.write(await user_kyc.panImage.read())
            ...
            cur.execute("insert into user_kyc(ownerName  , companyName  , businessType ,businessAddress ,panNumber ,aadharNumber ,isVerify ,panImage ,aadharImage ) values (%s,%s,%s,%s,%s,%s,%s,%s,%s)" , (
                user_kyc.ownerName , user_kyc.companyName , user_kyc.businessType , user_kyc.businessAddress , user_kyc.panNumber ,user_kyc.aadharNumber , False , str(image_path) , str(image_path2)
            ))
            conn.commit()
            cur.close()
